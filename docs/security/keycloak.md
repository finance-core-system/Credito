...
| `system-realm` | `credito-service-client` | confidential | Client Credentials | `credito-internal-api` |
...
`credito-service-client`는 사용자 브라우저가 아니라 서버 간 호출에 사용한다. 이 client는 secret을 안전하게 보관할 수 있는 내부 서비스가 사용하는 것을 전제로 하므로 confidential client와 client credentials 흐름을 사용한다.
...
이번 작업은 인증 서버의 기본 구성이다. 아직 서비스 쪽 보안 처리는 끝나지 않았다.

- 각 Spring 서비스의 JWT 검증
- issuer, audience, expiry 검증 구현
- service token의 `client_id`, `audience`, `scope` 검증
