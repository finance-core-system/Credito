...
@SpringBootTest(
    ...
    classes = {
        LendingServiceApplication.class,
        LendingServiceApplicationTests.TestEndpointConfiguration.class
    },
    properties = {
        "SERVER_PORT=0",
        "MTLS_ENABLED=false",
...
class LendingServiceApplicationTests {
    ...
    private static final String KEY_ID = "lending-service-test-key";
    ...
    static void resourceServerProperties(DynamicPropertyRegistry registry) {
        registry.add("CUSTOMER_REALM_ISSUER_URI", () -> "http://localhost:8085/realms/customer-realm");
    ...
    void applicationEndpointsRequireAuthentication() {
        ResponseEntity<String> response = restTemplate.getForEntity(
            "http://localhost:" + port + "/api/lendings",
            String.class);
    ...
    @Test
    void serviceTokenWithAllowedClientAndScopeCanAccessLendingsApi() throws Exception {
        HttpHeaders headers = new HttpHeaders();
        headers.setBearerAuth(serviceToken("gateway-service", "lendings.read"));
        ...
        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertEquals("lendings", response.getBody());
    }
    ...
    static class TestEndpointConfiguration {
        ...
        @Bean
        LendingProbeController lendingProbeController() {
            return new LendingProbeController();
        }
    }

    @RestController
    static class LendingProbeController {
